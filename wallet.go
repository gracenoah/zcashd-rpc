...
func (r FutureAddMultisigAddressResult) Receive() (btcutil.Address, error) {
	...
	return btcutil.DecodeAddress(addr, r.networkParams)
}
...
func (c *Client) GetNewAddressAsync(account string) FutureGetNewAddressResult {
	...
	respChan := c.sendCmd(cmd)
	return FutureGetNewAddressResult{
		respChan:      respChan,
		networkParams: c.networkParams,
	}
}
...
func (c *Client) GetRawChangeAddressAsync(account string) FutureGetRawChangeAddressResult {
	...
	respChan := c.sendCmd(cmd)
	return FutureGetRawChangeAddressResult{
		respChan:      respChan,
		networkParams: c.networkParams,
	}
}
...
func (c *Client) AddWitnessAddressAsync(address string) FutureAddWitnessAddressResult {
	...
	respChan := c.sendCmd(cmd)
	return FutureAddWitnessAddressResult{
		respChan:      respChan,
		networkParams: c.networkParams,
	}
}
...
func (r FutureGetAddressesByAccountResult) Receive() ([]btcutil.Address, error) {
	...
	for _, addrStr := range addrStrings {
		addr, err := btcutil.DecodeAddress(addrStr, r.networkParams)
		if err != nil {
			return nil, err
		}
